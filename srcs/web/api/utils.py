...
def get_message_str(code: str, level: str, **kwargs) -> str :
	...
	if code == "registration_success":
		return "You registered successfully. Welcome !"
	#
	...
	if split:
		...
		if split[0] == "invalid" or split[0] == "missing":
			return f"{split[0].capitalize()} arg: {' '.join([s for s in split[1:]])}."
...
def create_match(teams):
	if len(teams) == 0 or len(teams[0]) == 0:
		return None
	match_choice = teams[0][0].match_choice
	match = Match.objects.create(info=match_choice)
	players_count = 0
	for i in range(match_choice.teams_required()) :
		team_obj = Team.objects.create(match=match)
		team_obj.save()
		if i < len(teams) :
			count = 0
			for lobby in teams[i]:
				for lobby_player in lobby.members:
					entry = Entry.objects.create(player=lobby_player.player, team=team_obj)
					entry.save()
					count += 1
			players_count += count
			if count < match_choice.players_per_team() and match_choice.connectivity == Connectivity.LOCAL:
				for j in range(match_choice.players_per_team() - count):
					is_ai = match_choice.mode == GameMode.SOLO
					pseudo = 'IA' if is_ai else f'player{players_count + 1}'
					player, _ = Player.objects.get_or_create(user=None, pseudo=pseudo, is_ai=is_ai)
					entry = Entry.objects.create(player=player, team=team_obj)
					entry.save()
					players_count += 1
		else:
			for j in range(match_choice.players_per_team()):
				is_ai = match_choice.mode == GameMode.SOLO
				pseudo = 'IA' if is_ai else f'player{players_count + 1}'
				player, _ = Player.objects.get_or_create(user=None, pseudo=pseudo, is_ai=is_ai)
				entry = Entry.objects.create(player=player, team=team_obj)
